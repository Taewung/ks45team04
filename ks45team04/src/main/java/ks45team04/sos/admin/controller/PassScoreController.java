...
public class PassScoreController {
	...
	// 특정 과목정보 조회
	@GetMapping("/getSubInfoForLsPScore")
	@ResponseBody
	public LicenseSubject getSubInfoForLsPScore (@RequestParam(value="lsCode") String lsCode){
		LicenseSubject subInfoForLsPScore = subjectPassScoreService.getSubInfoForLsPScore(lsCode);
		log.info("특정 과목정보 조회 : {}", subInfoForLsPScore);
		return subInfoForLsPScore;
	}	
	// 자격증별 과목목록 조회
	@GetMapping("/getSubListForLsPScore")
	@ResponseBody
	public List<LicenseSubject> getSubListForLsPScore (@RequestParam(value="liCode") String liCode){
		List<LicenseSubject> subListForLsPScore = subjectPassScoreService.getSubListForLsPScore(liCode);	
		log.info("자격증별 과목목록 조회 : {}", subListForLsPScore);
		return subListForLsPScore;
	}
	// 과목합격기준점수 등록화면
